...
public class DaprInvokeFeignClient implements Client {
    ...
    public Response execute(Request request, Request.Options options) throws IOException {
        ...
        Mono<byte[]> result = switch (schema) {
            case "method" -> getResultFromInvokeHTTPMethodRequest(uri, request);
            case "binding" -> getResultFromInvokeBindingRequest(uri, request);
    ...
    private HttpExtension toHttpExtension(String method) throws IOException {
        return switch (method) {
            case "none" -> HttpExtension.NONE;
            case "put" -> HttpExtension.PUT;
            case "post" -> HttpExtension.POST;
            case "delete" -> HttpExtension.DELETE;
            case "head" -> HttpExtension.HEAD;
            case "connect" -> HttpExtension.CONNECT;
            case "options" -> HttpExtension.OPTIONS;
            case "trace" -> HttpExtension.TRACE;
            case "get" -> HttpExtension.GET;
            default -> throw new IOException("Method '" + method + "' is not supported");
        };
    }
